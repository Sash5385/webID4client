...
export default function ProfileTab({ user, profile, onProfileUpdate }) {
  ...
  const [adminProfile, setAdminProfile] = useState({ name: 'Інструктор', phone: '+380989225442', address: 'Київ, Верховинна 44' });

  useEffect(() => {
    getAdminSettings().then(s => {
      if (s?.profile) setAdminProfile(p => ({ ...p, ...s.profile }));
    }).catch(() => {});
  }, []);
      ...
      <div className="profile-section">
        ...
        {adminProfile.name && (
          <div style={{fontSize:14, color:'var(--text)', marginBottom:8, fontWeight:500}}>
            {adminProfile.name}{adminProfile.experience ? ` · ${adminProfile.experience} р. досвіду` : ''}
          </div>
        )}
        <div className="contact-phone-block">
          <div className="contact-btns">
            <a href={`tel:${adminProfile.phone}`} className="contact-btn contact-btn--call" aria-label="Зателефонувати">
            ...
            <a href={`https://t.me/${adminProfile.phone?.replace(/\D/g,'')}`} target="_blank" rel="noopener noreferrer" className="contact-btn contact-btn--tg" aria-label="Telegram">
            ...
            <a href={`viber://chat?number=%2B${adminProfile.phone?.replace(/\D/g,'')}`} className="contact-btn contact-btn--viber" aria-label="Viber">
            ...
            <a href={`https://wa.me/${adminProfile.phone?.replace(/\D/g,'')}`} target="_blank" rel="noopener noreferrer" className="contact-btn contact-btn--wa" aria-label="WhatsApp">
        ...
        {adminProfile.address && (
          <a
            href={`https://maps.google.com/?q=${encodeURIComponent(adminProfile.address)}`}
            target="_blank"
          ...
          >
            ...
            <div className="info">
              ...
              <div className="val">{adminProfile.address}</div>
            </div>
          </a>
        )}
      </div>
